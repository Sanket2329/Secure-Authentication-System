<div align="center">
  <h1>🎯 Engineering Approach & Interview Guide</h1>
  <p>A comprehensive breakdown of architectural decisions, design patterns, and security paradigms implemented in the Secure Authentication System.</p>
</div>
...
The overarching goal of this project was to build a production-quality, highly secure system while maintaining strict adherence to **Clean Architecture** and **SOLID principles**. 
...
* **Backend 1 (Custom)**: A highly-tuned Node.js/PostgreSQL REST API built completely from scratch.
...
* **The "Why"**: This demonstrates two distinct architectural skill sets: Building a secure system from scratch (Backend 1), and properly leveraging a modern Serverless/BaaS platform directly from the client (Backend 2).

### 1.2 Custom Backend Layering
The custom Node.js backend follows a strict layered architecture:
...
* ⚙️ **Service Layer (`TokenService`)**: Contains business logic (generating JWTs, cryptographic hashing).
* 💾 **Data Access Layer (`UserModel`, `FileModel`)**: Encapsulates raw SQL queries to PostgreSQL.
* **The "Why"**: This separation of concerns ensures maximum testability. We can swap PostgreSQL for MySQL or change the JWT library without rewriting our controllers.
...
1. **Access Tokens (JWT)**: Short-lived (15 minutes), stateless, and extremely fast. Sent in the JSON payload and used for authorization (`Bearer <token>`).
2. **Refresh Tokens (Opaque Strings)**: Long-lived (7 days), stateful, cryptographically random strings. Stored securely as **HttpOnly cookies** (immune to XSS) while their **SHA-256 hashes** are stored in the database.

* **The "Why"**: Pure JWTs are impossible to revoke instantly without complex blacklists. Pure sessions require a database hit on every API call. This hybrid approach gives us the speed of stateless JWTs with the instant revocability (during logout or compromise) of stateful sessions.
...
### 2.3 Data Isolation & IDOR Prevention
The `GET /api/files/:id` endpoint explicitly guards against Insecure Direct Object Reference (IDOR).
* **The Logic**: 
  1. Check if the file exists. If no -> `404 Not Found`.
  2. Check if `file.owner_id === req.user.userId`. If no -> `403 Forbidden`.
* **The "Why"**: Returning `404` for files that *do exist but don't belong to the user* (a common but flawed practice) prevents data leakage, but returning `403` when appropriate is far better for precise REST API semantics.
...
* 🗑️ **Foreign Key Cascades**: `ON DELETE CASCADE` ensures that if a user is deleted, all their files and refresh sessions are wiped automatically, preventing orphaned data.
* ⚡ **Indexes**: Added B-Tree indexes to `owner_id` and `user_id` to guarantee fast `O(log N)` lookups for relationship queries.

---

## ☁️ 4. Appwrite Implementation Nuances
...
* 🤖 **Automatic Features**: Appwrite natively handles password hashing (Argon2), automatic rate limiting, and session management. 
* 🛡️ **Document-Level Security**: When setting up the `Files` collection, we assigned permissions based on the specific `user`. Appwrite's database inherently filters documents at the query level. When `appwrite-adapter.js` calls `databases.listDocuments()`, Appwrite automatically only returns documents the user is authorized to see. This drastically reduces backend boilerplate code.
...
## 🚀 5. Future Technical Improvements (If Scaling)

In an interview, discussing what you *left out* and *why* is just as important as what you built:

1. **Redis Session Store**: Moving the stateful Refresh Tokens from PostgreSQL into Redis with native TTLs for faster, in-memory validation.
2. **MFA Integration**: Adding Time-Based One-Time Passwords (TOTP) or WebAuthn for biometric security.
